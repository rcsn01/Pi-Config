...
interface Stub {
	...
	guardian: { title: string; message: string }[];
...
function makeDeps(opts: { approve?: boolean; guardianAllow?: boolean } = {}): Stub {
	...
	const deps: EvaluateDeps = {
		...
		guardianReview: async (title, message) => {
			guardian.push({ title, message });
...
describe("evaluateToolCall", () => {
	...
	describe("auto-review mode", () => {
		it("delegates dangerous bash to the guardian and blocks on deny", async () => {
			...
			expect(s.guardian[0].title).toBe("Dangerous Command");
		...
		it("delegates external paths in commands to the guardian", async () => {
			const s = makeDeps({ guardianAllow: true });
			const d = await evaluateToolCall(
			...
			expect(s.guardian[0].title).toBe("External Path in Command");
			expect(d).toEqual({ action: "allow" });
		});

		it("delegates external writes to the guardian", async () => {
			const s = makeDeps({ guardianAllow: true });
			const d = await evaluateToolCall(
				{ toolName: "write", input: { path: "/etc/hosts" } },
				context({ mode: "auto-review" }),
