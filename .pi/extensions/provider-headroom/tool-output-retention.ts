...
export interface ToolOutputRetentionHost {
	appendEntry(customType: string, data: unknown): void;
	setRetrievalAvailable(available: boolean): void;
}

export interface CreateToolOutputRetentionOptions {
	overrides?: HeadroomExtensionOptions;
	host: ToolOutputRetentionHost;
...
function isToolResultMessage(message: RetentionMessage): message is ToolResultMessage {
...
function hydrateFromBranch(store: CcrStore, branch: readonly unknown[]): void {
...
function rewriteText(
	...
	if (!text || text.length < config.minChars || hasHeadroomMarker(text)) return { text, changed: false };
	const policy = toolPolicy(toolName, input, text, isError);
	if (policy.protected) return { text, changed: false };
	...
	if (!result.lossyChanged) return { text: result.text, changed: true };
	...
	if (decorated.length >= text.length) {
		...
		return lossless.changed ? { text: lossless.text, changed: true } : { text, changed: false };
	}
	if (entry) {
		store.store(entry);
		persist(entry);
	}
	return { text: decorated, changed: true, entry };
}

function contentWithTextBlock(content: ToolResultContent, blockIndex: number, text: string): ToolResultContent {
	return content.map((block, index) => (index === blockIndex && block.type === "text" ? { ...block, text } : block));
}

export function createToolOutputRetention({
	overrides = {},
	host,
	branch = [],
}: CreateToolOutputRetentionOptions): ToolOutputRetention {
	const store = new CcrStore();
	hydrateFromBranch(store, branch);
	const initialConfig = resolveConfig(overrides);
	host.setRetrievalAvailable(initialConfig.enabled && initialConfig.ccr && store.hasEntries());

	const persist = (entry: StoredCcrEntry): void => {
		if (!store.markPersisted(entry.hash)) return;
		host.appendEntry(CCR_ENTRY_TYPE, entry);
	};
	...
	return {
		rewriteFresh(input: FreshToolOutput, contextEntries: readonly unknown[]): FreshToolOutputRewrite {
			...
			const isReadCommandResult =
				(input.toolName === "bash" || input.toolName === "powershell") && isReadCommand(input.input.command);
			const protectedDetails = isReadCommandResult ? addProtectionHint(input.details) : input.details;
			...
			for (let blockIndex = 0; blockIndex < content.length; blockIndex++) {
				...
				changed = true;
				if (rewritten.entry) entries.push(rewritten.entry);
			}

			const details = entries.length > 0 ? addMetadata(protectedDetails, entries) : protectedDetails;
			if (!changed && details === input.details) return { changed: false, content, details };
			if (entries.length > 0) host.setRetrievalAvailable(true);
			return { changed: true, content, details };
		},

		projectHistory(inputMessages: readonly RetentionMessage[]): HistoryProjection {
			const config = resolveConfig(overrides);
			if (!config.enabled) {
				return { changed: false, messages: inputMessages as RetentionMessage[] };
			}
			...
			const messages = inputMessages.map((message) => {
				const hydratedEntries = isRecord(message) ? metadataEntries(message.details) : [];
				for (const entry of hydratedEntries) store.hydrate(entry);
				if (hydratedEntries.length > 0) hasCcr = true;
				if (!isToolResultMessage(message)) return message;
				const metadata = new Map(hydratedEntries.map((entry) => [entry.blockIndex, entry]));
				...
				if (config.dedupe && dedupeAllowed && content.length === 1 && firstBlock?.type === "text" && !metadata.has(0)) {
					...
					if (currentText.length >= config.dedupeMinChars && !hasHeadroomMarker(currentText)) {
						...
						const anchor = seenExact.get(key);
						if (anchor === currentText) {
							content = [{ ...firstBlock, text: "[Headroom: identical to an earlier tool result above.]" }];
							messageChanged = true;
						} else if (anchor === undefined && !messageChanged) {
							seenExact.set(key, currentText);
						}
					}
				}
			...
			if (hasCcr && config.ccr) host.setRetrievalAvailable(true);
			return {
				changed,
				messages: changed ? messages : inputMessages as RetentionMessage[],
			};
		},

		retrieve(rawHash: string): RetentionRetrieval {
			const hash = rawHash.trim().replace(/^hash=/i, "").toLowerCase();
			const entry = store.get(hash);
			return entry ? { found: true, hash, original: entry.original } : { found: false, hash };
		},
	};
}
