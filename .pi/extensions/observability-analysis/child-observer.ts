...
export default function childAnalysisObserver(pi: ExtensionAPI): void {
	const fd = Number(process.env.PI_ANALYSIS_RELAY_FD);
	...
	const relay = (event: Record<string, unknown>) => {
		...
		try {
			...
			if (frame.length > MAX_RELAY_MESSAGE_BYTES) return;
			let offset = 0;
			while (offset < frame.length) {
				const written = fs.writeSync(fd, frame, offset, frame.length - offset);
				if (written <= 0) throw new Error("analysis relay pipe closed");
				offset += written;
			}
		} catch {
			writable = false;
