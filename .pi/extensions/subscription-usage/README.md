...
and light-shade boxes (`░`) for the empty track. At the notify boundary the
output is ANSI-styled for the TUI: header lines and bars render bold bright
white, and the `N% used` share bold. The plain-text render functions and the
