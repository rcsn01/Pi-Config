...
- **Workflow run control module** — `workflows-engine/lib/workflow-run-control.ts` owns the session-local active-run registry, execute/restart dispatch, pause/stop lookup, operation settlement classification, settled-entry cleanup, and shutdown stop-and-drain behind `WorkflowRunControl`. The production singleton preserves one process-local command-control registry. It does not clean managed worktrees. `commands.ts` prepares runs and owns Pi notifications and result messages.
- **Workflow run lifecycle** — `workflows-engine/lib/workflow-run.ts` remains authoritative for one durable run's execution, pause and stop control, durable-key replay, agent admission, worktree result mapping and explicit cleanup, lifecycle writes, persistence ordering, and process-local lease behind `WorkflowRunHandle` and `WorkflowRunModule`. Managed-worktree cleanup recovers private run state, rejects active owners, validates recorded paths, preserves dirty trees, and returns per-target outcomes. Its coordinator serializes recovery, cleanup, and lifecycle event/projection operations by canonical run storage root and rejects a second active operation. It does not serialize unrelated persistence work such as artifact writes. The command adapter and run control module do not read or mutate reducer state.
- **Workflow run recovery** — `workflows-engine/lib/workflow-run-recovery.ts` opens validated private `RunState` above the `RunPersistence` adapter seam. It reads `events.jsonl` first. An existing log is authoritative, including an existing log with zero parsed events, which is an error. Only a missing log permits validated `state.json` fallback. Valid event-derived state repairs the projection on a best-effort basis. Workflow run lifecycle callers retain coordinator serialization around recovery.
- **Workflow run event module** — the pure in-process module in `workflows-engine/lib/workflow-run-events.ts` owns the engine event vocabulary, compatibility interpretation, live reduction, and durable replay behind a typed write interface and an open durable-read interface. Unknown events remain replayable no-ops after `run_created`; legacy Workflow and usage aliases remain event interpretation. The Workflow run lifecycle retains append, reduction, and projection ordering.
- **Workflow run persistence** — `events.jsonl` is canonical and `state.json` is its materialized projection. `workflow-run-state.ts` owns materialized state shape and read-model validation. `run-store.ts` owns file I/O, JSONL record-shape checks, and path safety, but not event interpretation. Recovery and lifecycle event/projection operations are serialized only within one Pi process. There is no cross-process lock, so two processes writing one run directory remain unsupported.
- **Workflow worktree policy** — the run lifecycle keeps normalization, replay reuse, preservation metadata, and explicit cleanup rules private. `preserve` is carried metadata; there is no automatic worktree removal. Explicit cleanup ignores `preserve`, refuses dirty trees, never force-removes, and can find admitted worktrees whose agents later fail. It shares the Git executor and artifact collector, but remains separate from `tools-worktree`.
...
- **Subagent child execution module** — the deep process-lifetime module for one
  ...
  only after asynchronous progress consumption settles. It receives prepared
  requests; Pi result rendering stays in the invocation adapter.
